...
Each year, Society for Exciting Computer Science Research (SECSR) organizes several conferences around 
the world. Lucy always picks one conference from that list that is hosted in a city she hasn't been to before, 
and if that leaves her with more than one option, she picks the conference that she thinks would be most 
relevant for her field of research.
...
If all of the offered conferences are hosted in cities that Lucy has visited before, the function should 
return 'No worthwhile conferences this year!' (Nothing in Haskell)
Example:
...
function conferencePicker(citiesVisited, citiesOffered) {
    const destination =  citiesOffered.filter(city => !citiesVisited.includes(city));
    return destination.length == 0 ? 'No worthwhile conferences this year!' : destination[0];
}

console.log(conferencePicker(['Tokyo','Madrid','Melbourne','Sydney','Rio De Janeiro','Saint Petersburg','Brisbane','Paris','Houston'],['Sydney','Chicago','Paris']));