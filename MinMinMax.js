...
the smallest number between the array bounds is -22. You may assume the input is well-formed.
...
function minMinMax(array) {
    ...
    let minBetweenTwoValue;

    for (let i = max; i >= min; i--) {
        if (!array.includes(i)) {
            minBetweenTwoValue = i;
        }
    }

    return [min, minBetweenTwoValue, max];
}
