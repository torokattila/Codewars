...
function rankOfElement(arr, i) {
  const leftPart = arr.slice(0, i).filter((num) => num <= arr[i]).length;
  const rightPart = arr.slice(i).filter((num) => num < arr[i]).length;
...
console.log(rankOfElement([3, 2, 3, 4, 1], 0));
console.log(rankOfElement([3, 2, 3, 4, 1], 1));
console.log(rankOfElement([3, 2, 3, 4, 1], 2));
