...
Alice and Bob are able to encode the motive as a number. Can you help them to remove numbers such that their 
...
reordering. For example if N = 2, and the input is [1,2,3,1,2,1,2,3], you take [1,2,3,1,2], drop the 
...
function deleteNth(arr, n) {
    ...
    const countedItems = arr.map((item) => {
        if (memo[item]) {
            memo[item]++;
        } else {
            memo[item] = 1;
    ...
    return countedItems.filter((item) => item !== undefined);
}
