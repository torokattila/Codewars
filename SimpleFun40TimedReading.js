...
Help the teacher figure out how many words the boy has read by calculating the number 
of words in the text he has read, no longer than maxLength.
...
function timedReading(maxLength, text) {
  ...
  const withoutSpecChars = text.replace(/[^a-zA-Z ]/g, '');

  return withoutSpecChars === ''
    ? 0
    : withoutSpecChars.split(' ').filter((word) => word.length <= maxLength)
        .length;
}
