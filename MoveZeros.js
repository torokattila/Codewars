/*Implement a function move_zeros which takes two arguments first an array of numbers arrNum and second a 
Boolean value isRight (default is true) and returns the array with all zero to right if 
isRight is true else to left if isRight is false.
...
function move_zeros(arrNum, isRight = true) {
    ...
    const withoutZeros = arrNum.filter((num) => num !== 0);

    return isRight ? [...withoutZeros, ...zeros] : [...zeros, ...withoutZeros];
}
