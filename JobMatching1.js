...
Let's give this a try. We'll create a function match which takes a candidate and a job, which will return a boolean 
indicating whether the job is a valid match for the candidate.
...
function match(candidate, job) {
    if (!candidate.minSalary || !job.maxSalary) throw new Error('Something went wrong');
    
	const wiggleRoom = candidate.minSalary * 0.10;

    return candidate.minSalary <= job.maxSalary + wiggleRoom;
}
