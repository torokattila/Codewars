/*Tap code is a way to communicate using a series of taps and pauses for each letter. In this kata, 
we will use dots . for the taps and whitespaces for the pauses.
...
export function tapCodeTranslation(text: string): string {
	...
	text.split('').forEach((char: string) => {
		if (coordinates[char]) {
			dotsArray.push('.'.repeat(coordinates[char][0]));
			dotsArray.push('.'.repeat(coordinates[char][1]));
		}
	});
