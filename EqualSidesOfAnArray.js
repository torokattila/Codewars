...
function sumOfLeftSide(array) {
  return array.length > 0
    ? array.reduce((accum, current) => accum + current)
    : 0;
}

function sumOfRightSide(array) {
  return array.length > 0
    ? array.reduce((accum, current) => accum + current)
    : 0;
}
...
function findEvenIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (sumOfLeftSide(arr.slice(0, i)) === sumOfRightSide(arr.slice(i + 1))) {
      equalIndices.push(i);
    }
  }

  return equalIndices.length === 0 ? -1 : equalIndices[0];
}
