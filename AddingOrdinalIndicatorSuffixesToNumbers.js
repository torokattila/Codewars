...
function numberToOrdinal(n) {
    ...
    const tensRemainder = n % 10;
    const hundredsRemainder = n % 100;

    if (tensRemainder === 1 && hundredsRemainder !== 11) return `${n}st`;
    if (tensRemainder === 2 && hundredsRemainder !== 12) return `${n}nd`;
    if (tensRemainder === 3 && hundredsRemainder !== 13) return `${n}rd`;

    return `${n}th`;
}

console.log(numberToOrdinal(1242));
