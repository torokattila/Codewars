...
function evenNumbersBeforeFixed(sequence, fixedElement) {
  let indexOfFixedElement = sequence.indexOf(fixedElement);
  ...
  if (!sequence.includes(fixedElement)) {
    ...
    for (let i = 0; i < sequence.length; i++) {
      if (i < indexOfFixedElement && sequence[i] % 2 == 0) {
        evenNumbersArray.push(sequence[i]);
      }
    }
  }

  return evenNumbersArray.length > 0 ? evenNumbersArray.length : 0;
}

console.log(evenNumbersBeforeFixed([2, 2, 2, 1], 3));
