...
function isPerfect(n, divisors = []) {
  ...
  let length = divisors.length;

  if (divisors[length - 1] * divisors[length - 1] === n) length--;
  ...
  const divisorsSum =
    divisors.slice(0, -1).length > 0
      ? divisors.slice(0, -1).reduce((accum, current) => accum + current)
      : 0;
