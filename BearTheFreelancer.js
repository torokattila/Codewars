/*
Bear the Freelancer charges clients on the hour, but he adjusts his rate depending on how close friends he is with his clients. 
...
Each array within the outer list is comprised of the number of hours worked, and the proximity to the client as a string, 
the possible values being 'Close Friend', 'Friend', 'Acquaintance', or any other string for the rest of his clients. 
...
function bearDollars(arr) {
  ...
  return arr
    .map((client) =>
      CLIENTS[client[1].toLowerCase()] &&
      client[1].toLowerCase() !== 'constructor'
        ? CLIENTS[client[1].toLowerCase()] * client[0]
        : client[0] * 125
    )
...
console.log(bearDollars([[1, 'constructor']]));
