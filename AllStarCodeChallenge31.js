...
He left Jesse an array of Instruction objects, which contain solutions, amounts, and the appropriate scientific 
instrument to use for each step in the "cook". The order of the instructions must be carried out in the EXACT order they're given. 
However, Jesse doesn't understand JavaScript and just needs things simplified!

Create a function called helpJesse() that accepts an array of Instruction objects as an argument. The function should convert 
each object into a string of the following format:
...
var recipe = [
	...
	new Instruction(
		...
		"Water",
		"Erlenmeyer Flask",
		"Do NOT mess this step up, Jesse!"
	)
];
...
function helpJesse(array) {
    ...
    array.forEach(item => {
        if (item.note) {
            resultArray.push(`Pour ${item.amount} mL of ${item.solution} into a ${item.instrument} (${item.note})`);
        } else {
            resultArray.push(`Pour ${item.amount} mL of ${item.solution} into a ${item.instrument}`);
        }
    });
