...
there was something more being said? Was it hard to figure out that unspoken meaning? Never again! 
Never will a secret go undiscovered. Find all duplicates from our message!
...
If the random tests repeat a word multiple times, it should show up in the secret message only once, based on the 
position of the first time it was duplicated.
...
function findSecretMessage(paragraph) {
    ...
    const withoutDotsAndExclMarks = paragraph.replace(/[.!]/g, '');
    withoutDotsAndExclMarks
        .toLowerCase()
        .split(' ')
        .forEach((word, index) => {
            if (duplicates[word]) {
