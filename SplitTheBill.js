...
The function should take one parameter: an object/dict with two or more name-value pairs which 
...
function splitTheBill(x) {
  const allSpending = Object.values(x);
  const averageOfGroupSpending = allSpending.length
    ? allSpending.reduce((accum, current) => accum + current) /
      allSpending.length
    : 0;
  const result = { ...x };

  for (key in result) {
    const difference = result[key] - averageOfGroupSpending;
    result[key] =
      difference % 1 !== 0 ? parseFloat(difference.toFixed(2)) : difference;
  }
