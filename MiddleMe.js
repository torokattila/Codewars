...
function middleMe(N, X, Y) {
	const repeatedString = Y.repeat(N);

	if (repeatedString.length / 2 % 1 !== 0) {
		return X;
	} else {
		return `${repeatedString.slice(0, repeatedString.length / 2)}${X}${repeatedString.slice(repeatedString.length / 2)}`;
	}
}
