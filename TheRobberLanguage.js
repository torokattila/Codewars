/*
The Robber Language (Rövarspråket) is a Swedish cant used by children to mislead people not 
familiar with the language. It's used by kids in several screenplays created by Astrid Lindgren.

The idea is that every consonant in a sentence is duplicated, with an o inserted in between 
(b becomes bob). Vowels are left untouched (a remains an a). It's quite hard for an untrained 
ear to understand a conversation encoded in this manner, especially if spoken rapidly.
...
The Assignment
Your task is to implement a function function robberEncode(sentence) which takes in a string 
sentence and returns the result, converted into robber language, as a string.
...
function robberEncode(sentence) {
  const upperPattern = /[BCDFGHJKLMNPQRSTVWXYZ]/;
  const lowerPattern = /[bcdfghjklmnpqrstvwxyz]/;

  return [...sentence]
    .map((char) => {
      if (upperPattern.test(char)) {
        char = `${char}O${char}`;
      } else if (lowerPattern.test(char)) {
        char = `${char}o${char}`;
      }
