...
Unlike conventional sorting algorithms that rearrange elements, Stalin Sort solves the problem of disorder radically: 
any element that violates the ascending order is shot and removed from the list.

Task
Implement the function stalin_sort / stalinSort, which accepts an array of integers and modifies it in-place, 
removing all elements that violate the ascending order relative to the previous surviving element.
...
export function stalinSort(array: number[]): void {
  ...
  for (let i = 1; i < array.length; i++) {
    if (array[i] >= array[survivorIndex]) {
      survivorIndex++;
      array[survivorIndex] = array[i];
    }
  }

  array.length = survivorIndex + 1;
}

console.log(stalinSort([3, 1, 4, 1, 5, 9, 2]));
console.log(stalinSort([1, 2, 3]));
