...
function countAdjacentPairs(searchString) {
    ...
    let lowerCaseArray = wordsArray.map(word => word.toLowerCase());
    let counter = 0;
    let i = 1;

    while (i < lowerCaseArray.length) {
        if (lowerCaseArray[i] === lowerCaseArray[i - 1]) {
            counter++;
            i++;
            while (lowerCaseArray[i] === lowerCaseArray[i - 1]) {
                i++;
            }
        } else {
            i++;
...
console.log(countAdjacentPairs(string));