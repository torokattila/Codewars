...
Array.prototype.square = function() {
    const squaredArray = this.map(number => number * number);

    return squaredArray;
};

Array.prototype.cube = function() {
    const cubedArray = this.map(number => Math.pow(number, 3));

    return cubedArray;
};

Array.prototype.average = function() {
    return this.length > 0 ? this.reduce((accum, current) => accum + current) / this.length : NaN;
};

Array.prototype.sum = function() {
    return this.length > 0 ? this.reduce((accum, current) => accum + current) : 0;
};
