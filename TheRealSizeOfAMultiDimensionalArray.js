...
Think of it as dimension-agnostic value counting. We want to know the total number of 
integers inside the array and we don't care for the dimensions.*/

function realSize(arrays) {
    let counter = 0;

    for (const innerItem of arrays) {
        if (Array.isArray(innerItem)) {
            counter += realSize(innerItem);
        } else {
            counter++;
        }
    }
