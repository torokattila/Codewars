...
Each next() call on the returned iterator object should generate the next 
element in the previously given array with length n. If it gets called more then n times, do NOT return undefined.
...
function* nextElementGenerator(array) {
  ...
  while (true) {
    if (array[increment] === undefined) {
      increment = 0;
    }
    yield array[increment];
    increment++;
  }
}
