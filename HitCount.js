...
Instead of just displaying the count value immediatley when the page loads, we want to 
create the effect of each digit cycling through its preceding numbers before stopping on the actual value.
...
function counterEffect(hitCount) {
	const resultArray = [];
    const characters = hitCount.split('');


    for (let i = 0; i < characters.length; i++) {
        const numberChar = Number(characters[i]);
        const innerArray = [];
