...
const alpha = { FAQHL: 1, POSUC: 2, VEXMY: 7, IBTNW: 6, KGDJZ: 5 };
// const alpha = { ABCDE: 1, FGHIJ: 2, KLMNO: 3, PQRST: 4, UVWXY: 5 };

function nameScore(name) {
    const alphaSeparateKeys = {};
    let score = 0;

    for (const key in alpha) {
        [...key].map((character) => {
            alphaSeparateKeys[character] = alpha[key];
        });
    }

    [...name].forEach((char) => {
        if (alphaSeparateKeys[char.toUpperCase()]) {
            score += alphaSeparateKeys[char.toUpperCase()];
        }
    });
