...
function mostMoney(students) {
    ...
    const studentsWithMoney = [];
    let areValuesTheSame = "all";

    students.forEach(student => {
        studentsWithMoney.push({
            name: student.name,
            money: (student.fives * 5) + (student.tens * 10) + (student.twenties * 20) 
        });
    });

    studentsWithMoney.sort((a, b) => b.money > a.money ? 1 : -1);

    studentsWithMoney.forEach(student => {
        if (student.money !== studentsWithMoney[0].money) {
            areValuesTheSame = "not same";
        }
    });

    return areValuesTheSame === "all" ? areValuesTheSame : studentsWithMoney[0].name;
}
