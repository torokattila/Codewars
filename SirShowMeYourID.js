...
function showMe(yourID) {
	if (!yourID.includes("-") && `${yourID.charAt(0).toUpperCase()}${yourID.substring(1).toLowerCase()}` === yourID) return true;
	if (!yourID.includes("-") && `${yourID.charAt(0).toUpperCase()}${yourID.substring(1).toLowerCase()}` !== yourID) return false;
	...
	return yourID
			...
			.map(section => {
				section = `${section
					.charAt(0)
					.toUpperCase()}${section.substring(1).toLowerCase()}`;
