...
function isbnConverter(isbn) {
  const withRemovedDigit = "978-" + isbn.slice(0, -1);
  const withRemovedHyphens = withRemovedDigit.replace(/[-]/g, "");
  const sumOfDIgits = withRemovedHyphens
    .split("")
    .map((digit, index) => {
      if (index % 2 === 0) {
  ...
  const checkDigit = sumOfDIgits % 10 === 0 ? 0 : 10 - (sumOfDIgits % 10);

  return withRemovedDigit + checkDigit;
}

console.log(isbnConverter("963-14-2164-3"));
