/*Learning to code around your full time job is taking over your life. You realise that in order to make significant 
steps quickly, it would help to go to a coding bootcamp in London.
...
Note that if s contains three instances of the letter 'l', that still scores three points, even though there 
is only one in the word sabbatical.

If the sum of the three parameters (as described above) is > 22, 
return 'Sabbatical! Boom!', else return 'Back to your desk, boy.'.*/

function sabb(s, val, happiness) {
	const sabbaticalChars = [...new Set('sabbatical'.split(''))];
	let charSum = 0;

	s.split('').forEach(char => {
		if (sabbaticalChars.includes(char.toLowerCase())) {
			charSum++;
		}
	});

	return charSum + val + happiness > 22
		? 'Sabbatical! Boom!'
