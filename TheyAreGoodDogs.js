...
Because they're good dogs. Over the weekend Bront has come in and hacked our system, 
lowering the ratings of dogs to below 10/10. Please help to fix Brant's bad system and give the dogs their original ratings. 
...
And the following rating: 11 return: 'This is Max99. She has one ear that is always s1ightly 
heigher than the other 11/10 wonky af'*/

function weRateDogs(str, rating) {
    const words = str.split(' ');
    const ratingWord = words.filter((word) => word.includes('/')).join('');
    const indexOfRatingWord = words.indexOf(ratingWord);
    const newRatingWord = `${rating}/${ratingWord.split('/')[1]}`;

    words[indexOfRatingWord] = newRatingWord;
