...
The text has a coating on it. When you buy the lottery ticket and then blow it off, you can see the text information below the coating.
...
function scratch(lottery) {
    ...
    newArray.forEach(innerArray => {
        animalsSetArray.push([...new Set([...innerArray])])
    });

    animalsSetArray.forEach(innerArray => {
        if (innerArray.length == 2) {
            prize += Number(innerArray[1]);
        }
    });
