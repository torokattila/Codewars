...
and their result is found by taking the average of the middle 3 solves (ie. the slowest and fastest times are disregarded, 
    and an average is taken of the remaining times).

In some competitions, the unlikely event of a tie situation is resolved by comparing Cuber's fastest times.

Write a function cube_times(times) that, given an array of floats times returns an array / tuple with the Cuber's 
result (to 2 decimal places) AND their fastest solve.
...
function cubeTimes(times) {
    ...
    const withoutSlowestAndFastest = times.filter(
        (time) => time !== fastest && time !== slowest
    );

    const average =
        withoutSlowestAndFastest.reduce((accum, current) => accum + current) /
        withoutSlowestAndFastest.length;

    return [Math.round(average * 100) / 100, fastest];
}
