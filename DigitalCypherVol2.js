...
function createModifiedKey(stringN, code) {
  let modifiedKey = '';

  if (stringN.length < code.length) {
    let i = 0;

    while (modifiedKey.length < code.length) {
      modifiedKey += stringN[i];
      i++;
      if (i === stringN.length) i = 0;
    }
  } else if (stringN.length === code.length) {
    modifiedKey = stringN;
  } else if (stringN.length > code.length) {
    modifiedKey = stringN.slice(0, code.length);
  }
...
function decode(code, n) {
  ...
  let stringN = `${n}`;
  let modifiedKey = createModifiedKey(stringN, code);

  for (let i = 1; i <= 26; i++) {
    pairs[i] = alphabet[i - 1];
  }
  for (let i = 0; i < code.length; i++) {
    const currentCode = code[i];

    result += pairs[currentCode - Number(modifiedKey[i])]
  }
