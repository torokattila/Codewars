...
function cyclops(n) {
	const numberInBinary = n.toString(2);
	if (numberInBinary.length % 2 === 0 || n === 1) return false;

	const onlyOneZero = numberInBinary.split('').filter(char => char === '0').length === 1;
	const onlyOneOne = numberInBinary.split('').filter(char => char === '1').length === 1;

	const middleCharacter = numberInBinary[Math.floor(numberInBinary.length / 2)];

	if (onlyOneZero) {
		return middleCharacter === '0';
	} else if (onlyOneOne) {
		return middleCharacter === '1';
	} else {
        return false;
    }
}

console.log(cyclops(33550334));
