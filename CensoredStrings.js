...
It will be very tedious to recover all these files manually, so your goal is to implement uncensor function 
that does the hard work automatically.
...
function uncensor(infected, discovered) {
    let counter = -1;
    let copyInfected = infected;
    infected = infected.split('');

    for (let i = 0; i < infected.length; i++) {
        if (infected[i] == '*') {
            counter++;
            copyInfected = copyInfected.replace(infected[i], discovered[counter]);
        }
    }
